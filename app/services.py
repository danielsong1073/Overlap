...
def get_book_metadata(title: str):
    ...
    if not docs:
        return None
    first_result = docs[0]
    official_title, work_id, cover_key, year = first_result.get("title"), first_result.get("key"), first_result.get("cover_edition_key"), first_result.get("first_publish_year")
...
def get_movie_metadata(title: str):
    ...
    if data.get("Response") == "False":
        return None
    official_title, imdb_id, poster, date = data.get("Title"), data.get("imdbID"), data.get("Poster"), data.get("Released")
...
def get_game_metadata(title: str):
    ...
    official_title, game_id, image, released = first_result.get("name"), first_result.get("id"), first_result.get("background_image"), first_result.get("released")
